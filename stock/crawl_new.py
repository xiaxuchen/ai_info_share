...
# New stocks to crawl (prefix based on code pattern)
NEW_STOCKS = [
    ('603598', '引力传媒', 'sh'),
    ('001330', '博纳影业', 'sz'),
    ('002915', '中欣氟材', 'sz'),
...
try:
    for code, name, prefix in NEW_STOCKS:
        full_code = prefix + code
        if name in all_data and len(all_data[name]) >= 30:
        ...
        if has_captcha(d):
            if not solve_slider(d):
                print('CAPTCHA blocked, stopping')
                break
        ...
        if not data:
            print('Failed to get posts'); continue
        ...
        print(f'  SAVED: {len(stock_posts)} posts, {total_c} comments')
